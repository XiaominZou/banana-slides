        ...
        with app.app_context():
            try:
                ...
                if proj.creation_type == "descriptions" and not proj.description_text:
                    yield _sse_event(
                        "error", {"message": "description_text is required"}
                    )
                    return
                ...
                if proj.creation_type not in ("outline", "descriptions"):
                    ...
                    if not idea_prompt:
                        yield _sse_event(
                            "error", {"message": "idea_prompt is required"}
                        )
                        return
                ...
                for page_data in ai_service.generate_outline_stream_with_agent(
                    project_context, language=language
                ):
                ...
                logger.info(
                    f"流式大纲生成完成: 项目 {project_id}, {len(pages_list)} 个页面, 降级: {agent_fallback}"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Session rollback failed: {rollback_exc}", exc_info=True
                    )
...
def generate_descriptions(project_id):
    ...
    try:
        ...
        # Create task
        task = Task(
            project_id=project_id, task_type="GENERATE_DESCRIPTIONS", status="PENDING"
        )
...
def generate_images(project_id):
    ...
    try:
        ...
        # Create task
        task = Task(
            project_id=project_id, task_type="GENERATE_IMAGES", status="PENDING"
        )
        task.set_progress({"total": len(pages), "completed": 0, "failed": 0})
...
def refine_outline(project_id):
    ...
    try:
        ...
        # Refine outline
        logger.info(
            f"开始修改大纲: 项目 {project_id}, 用户要求: {user_requirement}, 历史要求数: {len(previous_requirements)}, 联网搜索: {enable_web_search}"
        )
...
def refine_descriptions(project_id):
    ...
    try:
        ...
        # Refine descriptions
        logger.info(
            f"开始修改页面描述: 项目 {project_id}, 用户要求: {user_requirement}, 历史要求数: {len(previous_requirements)}, 联网搜索: {enable_web_search}"
        )
...
def create_ppt_renovation_project():
    ...
    try:
            ...
            try:
                ...
                for i, img in enumerate(images):
                    try:
                        ...
                        logger.error(
                            f"Failed to render page {i + 1} with pdf2image: {e}"
                        )
                        page_image_paths.append(None)
            except ImportError:
                raise ValueError(
                    "Neither PyMuPDF nor pdf2image is available for PDF rendering"
                )
        ...
        logger.info(
            f"Rendered {len(valid_pages)}/{len(page_image_paths)} page images from PDF"
        )
        ...
        if (
            pdf_page_width
            and pdf_page_height
            and pdf_page_width > 0
            and pdf_page_height > 0
        ):
            try:
                raw_ratio = (
                    f"{int(round(pdf_page_width))}:{int(round(pdf_page_height))}"
                )
                ...
                logger.warning(
                    f"Could not normalize PDF aspect ratio ({pdf_page_width}x{pdf_page_height}): {e}, keeping default 16:9"
                )
        ...
        for i, img_path in enumerate(page_image_paths):
            ...
            page = Page(
                project_id=project_id, order_index=len(pages_list), status="DRAFT"
            )
        ...
        # Create async task
        task = Task(project_id=project_id, task_type="PPT_RENOVATION", status="PENDING")
        task.set_progress(
            {
                "total": len(pages_list),
                "completed": 0,
...
def extract_style():
    ...
    try:
        ...
        try:
            ...
            return success_response({"style_description": style_description})
        finally:
            os.unlink(tmp_path)
