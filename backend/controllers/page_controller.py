...
def generate_page_description(project_id, page_id):
    ...
    try:
        ...
        db.session.rollback()
        return error_response("AI_SERVICE_ERROR", str(e), 500)


@page_bp.route("/<project_id>/pages/<page_id>/reuse-outline", methods=["POST"])
def reuse_outline_for_description(project_id, page_id):
    """
    POST /api/projects/{project_id}/pages/{page_id}/reuse-outline - 一键复用大纲生成描述
    """
    try:
        page = Page.query.get(page_id)
        ...
        # 保留elements
        if elements:
            desc_content["elements"] = elements
        ...
        return success_response(
            {
                "message": "Description generated from outline successfully",
                "page": page.to_dict(),
            }
        )
...
def generate_page_image(project_id, page_id):
    ...
    try:
        ...
        if not ref_image_path and not project.template_style:
            return bad_request(
                "No template image or style description found for project"
            )
        ...
        task = Task(
            project_id=project_id, task_type="GENERATE_PAGE_IMAGE", status="PENDING"
        )
...
def edit_page_image(project_id, page_id):
    ...
    try:
        ...
        task = Task(
            project_id=project_id, task_type="EDIT_PAGE_IMAGE", status="PENDING"
        )
        task.set_progress({"total": 1, "completed": 0, "failed": 0})
        db.session.add(task)
        db.session.commit()
...
def regenerate_renovation_page(project_id, page_id):
    ...
    try:
        ...
        logger.info(
            f"Regenerating renovation page {page.order_index + 1}: parsing PDF..."
        )
        filename = f"page_{page.order_index + 1}.pdf"
        _batch_id, md_text, extract_id, error_msg, _failed = (
            file_parser_service.parse_file(str(page_pdf_path), filename)
        )
        ...
        if not md_text.strip():
            return error_response(
                "PARSE_ERROR",
                f"Failed to extract content from page {page.order_index + 1}",
                400,
            )

        # Step 2: AI extract structured content
        logger.info(
            f"Regenerating renovation page {page.order_index + 1}: extracting content..."
        )
        ...
        if keep_layout:
            try:
                ...
                if page.cached_image_path:
                    ...
                    image_path = file_service.get_absolute_path(
                ...
                logger.error(
                    f"Layout caption failed for page {page.order_index + 1}: {e}"
                )
        ...
        page.set_outline_content({"title": title, "points": points})
        page.set_description_content(
            {"text": description, "generated_at": datetime.utcnow().isoformat()}
        )
        page.status = "DESCRIPTION_GENERATED"
        page.updated_at = datetime.utcnow()
